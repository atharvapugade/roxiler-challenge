  ...
  return (
    <div className="dashboard-container fadeIn">
      ...
      <div className="filter-bar">
        ...
        />
        <button className="btn-primary small" onClick={applySearch}>
          Search
        </button>
        <button className="btn-secondary small" onClick={clearSearch}>
          Clear
        </button>
      </div>
              ...
              sortedStores.map((store, idx) => {
                ...
                return (
                  <tr key={storeId}>
                    ...
                    <td>{store.name}</td>
                    <td>{store.address}</td>
                    <td>{store.overallRating?.toFixed(2) || "N/A"}</td>
                    <td>
                      <input
                        type="number"
                        min="1"
                    ...
                    <td>
                      {!store.userRating ? (
                        <button className="btn-primary small" onClick={() => submitRating(storeId)}>
                          Submit
                        </button>
                      ) : (
                        <button className="btn-secondary small" onClick={() => updateRating(storeId)}>
                          Modify
                        </button>
                      )}
                    </td>
